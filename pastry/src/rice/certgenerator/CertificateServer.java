...
public class CertificateServer {
    ...
    public Map arguments;
      ...
      if (uri.indexOf("?") > 0) {
        ...
        for (int i=0; i<args.length; i++) 
          if (args[i].indexOf("=") > 0) 
            arguments.put(args[i].substring(0, args[i].indexOf("=")), args[i].substring(args[i].indexOf("=")+1));
      }
		}
		public Request(String _uri) { uri = _uri; headers = new HashMap(); }
	...
	public static class GetRequest extends Request {
		public static String METHOD = "GET";
		public GetRequest(String u, Map h) { super(u,h); }
		public GetRequest(String u) { super(u); }
	}
	...
	public static class PostRequest extends Request {
		...
		public void setBody(StringBuffer _bodyBuf) { body = _bodyBuf.toString(); }
	}
    ...
    protected void die(Exception exc) {
      ...
      output.println("<h1>404 Not Found</h1>The URL <tt>" + uri 
		...
		protected void printFooter() { 
			output.println("<hr style=\"display: none;\"><div style=\"font-family: sans-serif; font-size: small; background-color: #eee; margin-top: 1.5em; padding: 1em; border: 1px solid #999; -moz-border-radius: 4px;\">CertificateServer Java server (<tt>" + getClass() + "</tt>)</div>");
		}
        ...
        boolean isPost = request.startsWith("POST ") || request.startsWith("PUT ");
				if (isGet || isPost) {
					...
					if (isPost) {
						...
						System.out.println("[CertificateServer.run] POST/PUT content-length = " + contentLength);
						if (contentLength > 0) {
							while (contentLength > 0) {
								...
								if (len >= 0) {
									...
									System.out.println("[CertificateServer.run] parsing form post: Content-length was "
    ...
    throws java.io.IOException, java.lang.IllegalAccessException 
	{
      ...
      //				InetAddress.getLocalHost()); // <-- will this deny connections from other hosts?  doubtful.
		}
    ...
    new Thread() {
      public void run() {
        try {
          ...
          Connection connection = DriverManager.getConnection("jdbc:mysql://joshua.cs.rice.edu/epost", "dsandler", "wombat");
          PreparedStatement insert = connection.prepareStatement("insert into cert_servers (hostname, alive) values ('" + hostname + "', NOW())");  
          
          try { insert.executeUpdate(); } catch (Exception e) {}
          
          connection.close();
          
          while (true) {
            try {
              Connection conn = DriverManager.getConnection("jdbc:mysql://joshua.cs.rice.edu/epost", "dsandler", "wombat");
              PreparedStatement update = conn.prepareStatement("update cert_servers set alive=NOW() where hostname='" + hostname + "'");  
            ...
            try { sleep(3*60*1000); } catch (Exception e) {}
          }
        } catch (Exception e) {
      ...
      try {
        if (request.uri.startsWith("/certificate")) {
          ...
          if ((username == null) || (password == null) || (ring == null))
            throw new Exception("Not all arguments present - " + username + " " + password + " " + ring);
          ...
          KeyPair caPair = (KeyPair) SecurityUtils.deserialize(SecurityUtils.decryptSymmetric((byte[]) ois.readObject(), SecurityUtils.hash("monkey".getBytes())));
          ois.close();
          
          KeyPair pair = SecurityUtils.generateKeyAsymmetric();
            
          IdFactory realFactory = new PastryIdFactory(env);
          Id ringId = realFactory.buildId(ring.substring(0,1).toUpperCase() + ring.substring(1).toLowerCase());
          ...
          PostUserAddress address = new PostUserAddress(new MultiringIdFactory(ringId, realFactory), username + "@" + ring.toLowerCase(), env);
          ...
          CertifiedNodeIdFactory.generateCertificate(new RandomNodeIdFactory(env).generateNodeId(), baos2, caPair.getPrivate());
          byte[] nodeID = baos2.toByteArray();
          
          Class.forName("com.mysql.jdbc.Driver");
          Connection connection = DriverManager.getConnection("jdbc:mysql://joshua.cs.rice.edu/epost", "dsandler", "wombat");
          PreparedStatement stmt = connection.prepareStatement("update certificates set cert_data=?, nodeid_data=? where name=? and ring=?");  
