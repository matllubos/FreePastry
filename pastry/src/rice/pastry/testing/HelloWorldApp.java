...
public class HelloWorldApp extends PastryAppl {
  ...
  public void sendRndMsg(RandomSource rng) {
    ...
    if (Log.ifp(5))
      System.out.println("Sending message from " + getNodeId()
          + " to random dest " + rndid);
  ...
  public void leafSetChange(NodeHandle nh, boolean wasAdded) {
    if (Log.ifp(5)) {
      System.out.print("In " + getNodeId() + "'s leaf set, " + "node "
          + nh.getNodeId() + " was ");
      if (wasAdded)
  ...
  public void routeSetChange(NodeHandle nh, boolean wasAdded) {
    if (Log.ifp(5)) {
      System.out.print("In " + getNodeId() + "'s route set, " + "node "
          + nh.getNodeId() + " was ");
      if (wasAdded)
        System.out.println("added");
      else
