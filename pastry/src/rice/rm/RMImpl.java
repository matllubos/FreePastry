 ...
 * @(#) RMImpl.java
 * 
 * This (Replica Manager Module) implements the RM interface. This runs as a
 * Application which dynamically (in the presence of nodes joining and leaving
 * the network) maintains the invariant that objects are replicated over the
 * requested number of replicas. This module has been built over the CommonAPI
 * defined in "Towards a Common API for Structured Peer-to-Peer Overlays." Frank
 * Dabek, Ben Zhao, Peter Druschel, John Kubiatowicz and Ion Stoica. In
 * Proceedings of the 2nd International Workshop on Peer-to-peer Systems
 * (IPTPS'03) , Berkeley, CA, February 2003. Hence the implementation is meant
 * to be seamlessly portable to the other existing P2P protocols.
 * 
 * @version $Id$
 * 
...
public class RMImpl extends CommonAPIAppl implements RM {
  ...
  /**
   * This flag is set to true when this RM substrate is ready. The RM substrate
   * is ready when the underlying Pastry node is ready.
   */
  private boolean m_ready;

  /**
   * This will be incremented for every message that this node sends remotely.
   * Specifically, incremented for every RMRequestKeysMsg, RMResponseKeysMsg it
   * sends. This is to aid in the debugging phase.
   */
  public int m_seqno;
   ...
   * That is if a Timeout occurred for a RMRequestKeysMsg, then the subsequent
   * RMRequestKeysMsg, that will be resent will have the SAME eventId but an
   * increased value of seqNo.
   */
  public int m_eId;

  /**
   * This represents the range of object identifiers in the Id space for which
   * this node (by virtue of its position in the Id Space relative to the
   * positions of the other Pastry nodes) is an i-root (0 <=i <=k).
   */
  public IdRange myRange;

  /**
   * rFactor stands for the number of additional replicas for an object. The
   * primary replica is denoted a 0-root, other replicas are denoted as i-root
   * (1 <=i <=rFactor).
   */
  public int rFactor;
  ...
  /**
   * This hashtable is keyed by the NodeId of the node to whom this local node
   * is requesting for a set of keys in the Keys Exchange protocol. Since
   * potentially the number of keys in a particular range could be very high,
   * asking for the entire set of keys in the intial range could result in huge
   * packet sizes. In order to circumvent this problem, we devise some
   * strategies in the Keys Exchange protocol to split this range into smaller
   * ranges and issue requests for these subranges one at a time. This requires
   * state in the form of this hashtable to maintain the list f pending ranges,
   * for which a request for the key set is yet to be issued.
   */
  ...
  /**
   * This value represents the maximum size of the keySet corresponding to a
   * requested id range that we would like to fit in a single message. As a
   * result of this, we need some strategies to split ranges in the Keys
   * Exchange Protocol.
   */
  public static int MAXKEYSINRANGE = 1024;

  /**
   * This value represents the splitting factor by which a range is split into
   * in the Keys Exchange Protocol. The splitting method is recursive binary
   * splitting until we split the range into a total of SPLITFACTOR parts.
   */
  public static int SPLITFACTOR = 16;

  /**
   * This is the per entry state corresponding to the m_pendingRanges hashtable
   * that we maintain in the Keys Exchange Protocol.
   */
  public static class KEPenEntry {
    ...
    /**
     * Equality is based only on the reqRange part, does not care about the
     * numKeys argument.
     */
  ...
  public RMImpl(PastryNode pn, RMClient _app, int _rFactor, String instance) {
    ...
    if (isReady()) {
      ...
      //System.out.println(" Constructor::Need to do initial fetching of keys
      // from " + getNodeId());
      ...
      if (getPastryNode() instanceof DistPastryNode) {
        ...
        msg = new RMMaintenanceMsg(getNodeHandle(), getAddress(),
            getCredentials(), m_seqno++, thePastryNode.getEnvironment());
        getPastryNode().scheduleMsgAtFixedRate(msg,
            RMMaintenanceMsg.maintStart * 1000,
            RMMaintenanceMsg.maintFreq * 1000);
  ...
  /**
   * Returns true if the RM substrate is ready. The RM substrate is ready when
   * underlying PastryNode is ready.
   */
   ...
   * 
   * @param toNode the node with whom the local node is communicating this is
   *          the key for this entry in the hashtable
   * @param eId the event Id associated with this RMRequestKeys msg.
   */
  public void addPendingEvent(NodeId toNode, int eId) {
    //System.out.println("At " + getNodeId() + "addPendingEvent( " + toNode + "
    // , " + eId + " ) ");
  ...
  public void removePendingEvent(NodeId toNode, int eId) {
    //System.out.println("At " + getNodeId() + "removePendingEvent( " + toNode
    // + " , " + eId + " ) ");
    ...
    if (m_pendingEvents.containsKey(toNode)) {
      ...
      if (setOfEvents.contains(new Integer(eId))) {
        ...
        // Possible cause is message duplication, or Timeout expiring while the
        // response msg
        // is int transit
        //System.out.println("At " + getNodeId() + "Warning1: In
        // removePendingEvent(" + toNode + "," + eId + " ): Should not happen");
      }
    } else {
      // Possible cause is message duplication, or Timeout expiring while the
      // response msg
      // is int transit
      //System.out.println("At " + getNodeId() + "Warning2: In
      // removePendingEvent(" + toNode + "," + eId + " ): Should not happen");
  ...
  public boolean isPendingEvent(NodeId toNode, int eId) {
    //System.out.println("At " + getNodeId() + "isPendingEvent( " + toNode + "
    // , " + eId + " ) ");
  ...
  public void addPendingRange(NodeId toNode, IdRange reqRange) {
    //System.out.println("At " + getNodeId() + "addPendingRange( " + toNode + "
    // , " + reqRange + " ) ");
  ...
  public void updatePendingRange(NodeId toNode, IdRange reqRange, int numKeys) {
    //System.out.println("At " + getNodeId() + "updatePendingRange( " + toNode
    // + " , " + reqRange + " , " + numKeys + " ) ");
    RMImpl.KEPenEntry entry = new RMImpl.KEPenEntry(reqRange);
    Vector setOfRanges;
    if (m_pendingRanges.containsKey(toNode)) {
      ...
      if (setOfRanges.contains(entry)) {
        ...
        //System.out.println("At " + getNodeId() + "Warning1: In
        // updatePendingRange(" + toNode + "," + reqRange + " , " + numKeys + "
        // ): Should not happen");
  ...
  public void removePendingRange(NodeId toNode, IdRange reqRange) {
    //System.out.println("At " + getNodeId() + "removePendingRange( " + toNode
    // + " , " + reqRange + " ) ");
    ...
    if (m_pendingRanges.containsKey(toNode)) {
      ...
      if (setOfRanges.contains(new RMImpl.KEPenEntry(reqRange))) {
        ...
        // Possible cause is message duplication
        //System.out.println("At " + getNodeId() + "Warning1: In
        // removePendingRange(" + toNode + "," + reqRange + " ): Should not
        // happen");
      }
    } else {
      // Possible cause is message duplication
  ...
  /**
   * Iterates over the list of pending Ranges and splits the ranges if the
   * expected number of keys in a range is greater than MAXKEYSINRANGE.
   * 
   * @param toNode the node with which this local was communicating in the keys
   *          exchange protocol.
   */
  ...
  /**
   * Returns a Vector of the split parts of this range. The splitting method is
   * recursive binary spliting until we get a total SPLITFACTOR number of
   * subranges from the intitial range.
   * 
  ...
  /**
   * Gets the list of pending Ranges corresponding to a node that we are
   * communicating to in the Keys Exchange Protocol.
   * 
   * @param toNode the node with which this local was communicating in the keys
   *          exchange protocol.
   */
  ...
  /**
   * Prints the list of pending Ranges corresponding to a node that we are
   * communicating to in the Keys Exchange Protocol.
   * 
   * @param toNode the node with which this local was communicating in the keys
   *          exchange protocol.
  ...
  public void notifyReady() {
    if (app != null) {
      //System.out.println("notifyReady called for RM application on" +
      // getNodeId());
      m_ready = true;
      myRange = range(getLocalHandle(), rFactor, getNodeId(), true);
      app.rmIsReady(this);
      //System.out.println("MyRange= " + myRange);
      //System.out.println("notifyReady()::Need to do initial fetching of keys
      // from " + getNodeId());
  ...
  /**
   * This is the periodic maintenance protocol which removes stale objects as
   * well as checks to see if there is any missing object. This call is invoked
   * by the RMMaintenanceMsg message that is scheduled to be periodically
   * invoked at the local node.
   */
  ...
  public void update(NodeHandle nh, boolean wasAdded) {
    ...
    //System.out.println("leafsetChange(" + nh.getNodeId() + " , " + wasAdded +
    // ")" + " at " + getNodeId());
  ...
  /**
   * This function determines the nodes to which the local node requests for
   * keys.
   * 
   * @param rangeSet - contains a list of IdRanges that this node will request
   *          for.
   */
  ...
  /**
   * We send RMRequestKeys messages to the the nodes in the 'set' asking for
   * keys in the ranges specified in the 'rangeSet'. Additionally, in order to
   * implement the TIMEOUT mechanism to handle loss of RMRequestKeysMsg, we wrap
   * the RMRequestKeysMsg in a RMTimeoutMsg which we schedule on the local node
  ...
  public void sendKeyRequestMessages(NodeSet set, Vector rangeSet) {
    ...
    for (int i = 0; i < set.size(); i++) {
      ...
      int eId = m_eId++;
      msg = new RMRequestKeysMsg(getLocalHandle(), getAddress(),
          getCredentials(), m_seqno++, rangeSet, eId);

      if (getPastryNode() instanceof DistPastryNode) {
        // We will also wrap this message in order to implement the TIMEOUT
        // mechanism.
        RMRequestKeysMsg.WrappedMsg wmsg = new RMRequestKeysMsg.WrappedMsg(msg,
            toNode);
        RMTimeoutMsg tmsg = new RMTimeoutMsg(getNodeHandle(), getAddress(),
            getCredentials(), m_seqno++, wmsg);
        getPastryNode().scheduleMsg(tmsg, RMRequestKeysMsg.TIMEOUT * 1000);
        addPendingEvent(toNode.getNodeId(), eId);
      }

      //System.out.println("At " + getNodeId() + "sending RequestKeys msg to "
      // + toNode.getNodeId());
  ...
  /**
   * Called by client(RMClient) to notify the RM substrate of the presence of a
   * key corresponding to a object that was 'recently' inserted at itself. The
   * RM substrate algorithm is designed on a Pull model. This call however gives
   * the RM substrate to implement the Push model if it desires so in future.
   * The current implementation this method is non-operational since we believe
   * that the Pull model behaves sufficiently well.
   */
  ...
  /**
   * Called by client(RMClient) to enable optimizations to route to the nearest
   * replica. Should be called by client in the context of the forward method of
   * a lookup message. Should only be called if the local client does not have
   * the desired object. This call could change the nextHop field in the
   * RouteMessage.
   */
  public void lookupForward(RouteMessage msg) {
    ...
    for (int i = 1; i < set.size(); i++) {
      ...
      int base = thePastryNode.getRoutingTable().idBaseBitLength;

      // if handle is numerically closer or shares a longer prefix
      if ((handle.getNodeId().indexOfMSDD(target, base) < getNodeId()
          .indexOfMSDD(target, base))
          || ((handle.getNodeId().indexOfMSDD(target, base) == getNodeId()
              .indexOfMSDD(target, base)) && (handle.getNodeId().distance(
              target).compareTo(getNodeId().distance(target)) < 0))) {
    ...
    // We will change the nextHop Field of the RouteMessage to reflect this
    // closest Replica
